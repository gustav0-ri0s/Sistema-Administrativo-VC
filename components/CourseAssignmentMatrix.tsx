...
const CourseAssignmentMatrix: React.FC = () => {
  ...
  const filteredAssignments = assignments.filter(as => {
    const d = docentes.find(doc => doc.id === as.profileId);
    ...
    return (
      ...
      (c?.name || `${c?.grade} ${c?.section}`).toLowerCase().includes(search) ||
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredAssignments.map((as) => {
            const docente = docentes.find(p => p.id === as.profileId);
            const classroom = classrooms.find(c => c.id === as.classroomId);
            const area = areas.find(a => a.id === as.courseId);

            return (
              <div key={as.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:border-transparent transition-all group relative overflow-hidden">
                <div className="absolute top-0 right-0 w-24 h-24 -mr-12 -mt-12 bg-[#57C5D5]/5 rounded-full" />

                <div className="flex justify-between items-start relative z-10 mb-4">
                  <div className="flex flex-col gap-1.5">
                    <span className="text-[10px] font-black text-[#57C5D5] uppercase tracking-[0.2em] bg-[#57C5D5]/5 px-3 py-1 rounded-full w-fit border border-[#57C5D5]/10">
                      {classroom?.name || (classroom ? `${classroom.grade} ${classroom.section}` : 'N/A')}
                    </span>
                    <h4 className="text-sm font-black text-slate-900 leading-tight mt-1 line-clamp-2 min-h-[2.5rem] tracking-tight">{area?.name}</h4>
                  </div>
                  <button
                ...
                <div className="space-y-4 pt-6 border-t border-slate-50 relative z-10">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="overflow-hidden">
                      <p className="text-[11px] font-black text-slate-700 uppercase truncate tracking-tight">{docente?.full_name}</p>
                      <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest leading-none mt-1">Docente Asignado</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black text-slate-500">{as.hoursPerWeek} HRS Lectivas</span>
                    </div>
                    <div className="px-2.5 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[8px] font-black uppercase tracking-[0.2em] border border-emerald-100">
                      ACTIVO
                    </div>
                  </div>
                </div>
