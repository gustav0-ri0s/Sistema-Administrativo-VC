...
/* Fixed: Updated imports to use mockCurricularAreas instead of mockCourses */
import { mockProfiles, mockClassrooms, mockCurricularAreas, mockCourseAssignments } from '../services/mockData';
import { CourseAssignment, UserRole } from '../types';
import { ClipboardList, Plus, User, Book, MapPin, Trash2, Clock } from 'lucide-react';
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {assignments.map((as) => {
          ...
          const classroom = mockClassrooms.find(c => c.id === as.classroomId);
          /* Fixed: Use mockCurricularAreas instead of mockCourses */
          ...
          return (
            <div key={as.id} className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl transition-all group border-l-8 border-l-[#57C5D5]">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h4 className="text-xl font-black text-slate-900 leading-tight">{course?.name}</h4>
                  <p className="text-[10px] font-black text-[#57C5D5] uppercase tracking-widest mt-2 bg-[#57C5D5]/5 w-fit px-2 py-0.5 rounded-full">{classroom?.name}</p>
                </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xs font-black text-slate-800 uppercase leading-none">{docente?.full_name}</p>
