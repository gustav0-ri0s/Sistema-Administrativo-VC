...
const CourseAssignmentMatrix: React.FC = () => {
  ...
  const [tutorClassroomId, setTutorClassroomId] = useState<string | null>(null);

  useEffect(() => {
    if (selectedTeacher) {
      setTutorClassroomId(selectedTeacher.tutor_classroom_id ? selectedTeacher.tutor_classroom_id.toString() : null);
    } else {
      setTutorClassroomId(null);
    }
  }, [selectedTeacher]);
  ...
  const handleSaveAssignments = async () => {
    ...
    try {
      setIsSaving(true);
      // Delete existing assignments for this teacher in selected classrooms
      const existingAssignments = assignments.filter(a => a.profileId === selectedTeacher.id);

      // We only clear assignments for the classrooms we are currently editing (selectedClassroomIds) 
      // OR do we overwrite everything for this teacher? 
      // The previous logic was likely additive or replacement. 
      // Let's assume we replace assignments for the SELECTED classrooms.

      const toDelete = existingAssignments.filter(a => selectedClassroomIds.includes(a.classroomId.toString()));
      for (const a of toDelete) {
        await courseAssignmentService.delete(a.id);
      }
      ...
      showToast('success', 'Carga horaria y tutoría guardadas correctamente', 'Éxito');
      resetModal();
      loadData();
    } catch (error) {
      console.error('Save error:', error);
      showToast('error', 'Error al guardar asignaciones', 'Error');
    } finally {
      setIsSaving(false);
    }
  };
                              ...
                              <div className="flex items-center gap-2 mb-4">
                                {room?.tutorId && room.tutorId !== selectedTeacher?.id ? (
                                  <div className="flex items-center gap-3 px-4 py-2 rounded-xl bg-slate-100 border-2 border-slate-200 opacity-60">
                                    <div className="w-5 h-5 rounded flex items-center justify-center bg-slate-300">
                                      <User className="w-3 h-3 text-white" />
                                    </div>
                                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">
                                      Tutor ya asignado: {room.tutorName}
                                    </span>
                                  </div>
                                ) : (
                                  <label className={`flex items-center gap-3 cursor-pointer px-4 py-2 rounded-xl border-2 transition-all ${tutorClassroomId === cid ? 'bg-amber-50 border-amber-200 shadow-md shadow-amber-500/10' : 'bg-transparent border-transparent hover:bg-slate-100'}`}>
                                    <div className={`w-5 h-5 rounded flex items-center justify-center transition-colors ${tutorClassroomId === cid ? 'bg-amber-500' : 'bg-slate-200'}`}>
                                      {tutorClassroomId === cid && <Check className="w-3.5 h-3.5 text-white" />}
                                    </div>
                                    <input
                                      type="checkbox"
                                      checked={tutorClassroomId === cid}
                                      onChange={(e) => setTutorClassroomId(e.target.checked ? cid : null)}
                                      className="sr-only"
                                    />
                                    <span className={`text-[10px] font-black uppercase tracking-widest ${tutorClassroomId === cid ? 'text-amber-700' : 'text-slate-400'}`}>
                                      {tutorClassroomId === cid ? 'Eres Tutor de este Aula' : 'Asignar como Tutor de Aula'}
                                    </span>
                                  </label>
                                )}
                              </div>
